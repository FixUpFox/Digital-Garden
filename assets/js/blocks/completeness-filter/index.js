	...
	parent: ['digital-garden/container', 'core/group', 'core/row'],
		...
		edit: function CompletenessFilterEdit() {
			const blockProps = useBlockProps({
				className: 'digital-garden-block-preview digital-garden-completeness-filter__preview',
			});

			const localized = window.digitalGardenCompleteness || {};
			const localizedOptions = Array.isArray(localized.options) ? localized.options : [];
			const options =
			...
			return el(
				...
				el(
					'div',
					{ className: 'digital-garden-completeness-filter', 'aria-hidden': 'true' },
					el(
						'select',
						{
